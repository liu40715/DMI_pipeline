...
# Redis 連線設定，依你環境調整 host/port/db
...
def extract_data(ti, algorithm_module: str = "processor.extract.readCSV", algorithm_func: str = "test_csv", **algo_kwargs):
    try:
        logger.info("執行 extract_data (資料載入層)")

        algo_module = importlib.import_module(algorithm_module)
        func = getattr(algo_module, algorithm_func)
        logger.info(f"呼叫演算法 '{algorithm_func}' 來自模組 '{algorithm_module}'")
...
def transform_data(ti, algorithm_module="processor.transform.algos_1", algorithm_func="aggregate_product_data", **algo_kwargs):
    try:
...
def load_data(ti, algorithm_module: str = "processor.load.saveCSV", algorithm_func: str = "test_csv", **algo_kwargs):
    try:
